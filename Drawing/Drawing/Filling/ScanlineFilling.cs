...
namespace Drawing.Filling
{
    ...
    public class ScanlineFilling
    {
        ...
        public ScanlineFilling(Action<int, int> putPixel) 
        {
            PutPixel = putPixel;
        }
        ...
        private void buildEdgeTable(List<Edge> ETBucket, List<Edge> edges)
        {
            ...
            while (edges.Count != 0)
            {
                ...
                // find the bucket to add edge in
                int bucketIndex = -1;
                ...
                {
                    Edge bucketToAdd = ETBucket[bucketIndex];
                    while (bucketToAdd.next != null) 
                    {
                        bucketToAdd = bucketToAdd.next;
                    }
                    bucketToAdd.next = minEdge;
                }
        ...
        private void addActiveEdgeTable(int y, List<Edge> AET, List<Edge> ETBucket)
        {
            ...
            // if exists, add it to AET
            if (y_index >= 0)
            {
                Edge bucketEdge = ETBucket[y_index];
                while (bucketEdge != null)
                {
                    AET.Add(bucketEdge);
                    bucketEdge = bucketEdge.next;
                }
